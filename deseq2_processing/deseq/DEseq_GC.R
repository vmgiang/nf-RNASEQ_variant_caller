setwd("/home/giang/R/DEG/DEG_GC/")
library("dplyr")
library("DESeq2")

# import read count
salmon_count<- read.csv("GC_salmon_count.csv",header = T)
View(salmon_count)
# inport meta data
coldata<-read.csv("gc_meta_datadata.csv")
View(coldata)
...
res <- results(dds)
res
res <- results(dds, name= "condition_normal_vs_GC")
res <- results(dds, contrast=c("condition","normal","GC"))

resultsNames(dds)
resLFC <- lfcShrink(dds, coef="condition_normal_vs_GC", type="apeglm")
...
df2 <- na.omit(df)
upGenes <- rownames(df2[df2$log2FoldChange > 10 & df2$pvalue < 0.05,])
downGenes <- rownames(df2[df2$log2FoldChange < -10,])
...
# filtering
sig.df<- sig.df[(sig.df$baseMean>250)&(abs(sig.df$log2FoldChange)>6),]
...
df$diffexpressed[df$log2FoldChange > 10 & df$pvalue < 0.05] <- "UP"
# if log2Foldchange < -0.6 and pvalue < 0.05, set as "DOWN"
df$diffexpressed[df$log2FoldChange < -10 & df$pvalue < 0.05] <- "DOWN"
...
ggplot(data = df, aes(x = log2FoldChange, y = -log10(pvalue), col = diffexpressed, label = delabel)) +
  ...
  coord_cartesian(ylim = c(0, 60), xlim = c(-30, 30)) + # since some genes can have minuslog10padj of inf, we set these limits
  labs(color = 'Expression', #legend_title, 
       x = expression("log"[2]*"FC"), y = expression("-log"[10]*"p-value")) + 
  scale_x_continuous(breaks = seq(-30, 30, 5)) + # to customise the breaks in the x axis
  ggtitle('Dysregualted genes in Gastric cancer samples vs Normal samples') + # Plot title 
